            ...
            <table className="items-center mx-auto w-full border-collapse bg-white rounded shadow-lg">
                <thead>
                    <tr>
                        ...
                        <th className="w-[40%] px-2 sm:px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Package name
                        </th>
                            ...
                            <td className="border-t-0 px-2 sm:px-3 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap sm:p-4 py-2">
                                <div className="flex">
                                    {packageItem.state === 'approved' ? 
                                        <button onClick={() => handleChangeStatus(packageItem._id, packageItem.state)} className="border-none outline-none flex justify-center mr-2 w-[50%] px-2 py-2 text-white items-center cursor-pointer rounded bg-red-500"><TiCancel /><p className="lg:block lg:ml-2 hidden">Reject</p></button>
                                    : packageItem.state === 'rejected' ? <button onClick={() => handleChangeStatus(packageItem._id, packageItem.state)} className="border-none outline-none flex justify-center mr-2 w-[50%] px-2 py-2 text-white items-center cursor-pointer rounded bg-green-500"><TiTickOutline /><p className="lg:block lg:ml-2 hidden">Approve</p></button>
                                    : <div className="flex w-full justify-between">
                                        <button onClick={() => handleChangeStatus(packageItem._id, "rejected")} className="border-none outline-none flex justify-center w-[50%] py-2 text-white items-center cursor-pointer rounded bg-green-500"><TiTickOutline /><p className="lg:block lg:ml-2 hidden">Approve</p></button>
