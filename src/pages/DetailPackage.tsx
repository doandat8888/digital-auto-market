                        ...
                        <div className="sm:w-[80%] lg:w-[80%] xl:w-[70%] w-[100%] bg-slate-200 mt-2 px-2 md:px-6 sm:py-6 py-2 rounded-lg">
                                ...
                                <div className="sm:w-[80%] w-[70%] sm:pl-3 pl-1 flex flex-col sm:ml-4 ml-2">
                                    <div className="items-center sm:flex">
                                        <div className="flex justify-between">
                                            ...
                                            <div className="flex">
                                                {canEdit &&
                                                    <div onClick={() => updatePackage(packageDetail)} className="ml-2 py-1 px-1 round flex items-center cursor-pointer hover:opacity-60 text-black select-none border border-gray-500
                                                        rounded-lg"><AiOutlineEdit /><p className="text-[8px] hidden sm:block sm:text-[10px] ml-1">Update</p> 
                                                    </div>
                                                }
                                                {canEdit &&
                                                    <div onClick={() => onRemovePackage()} className="ml-2 py-1 px-1 round flex items-center cursor-pointer hover:opacity-60 text-black select-none border border-gray-500
                                                        rounded-lg"><AiOutlineDelete /><p className="text-[8px] hidden sm:block sm:text-[10px] ml-1">Delete</p> 
                                                    </div>
                                                }
                                            </div>
                                    ...
                                    <div className="flex justify-between sm-text-[14px] lg:text-[16px]">
                                        ...
                                        <select onChange={(event: React.ChangeEvent<HTMLSelectElement>) => handleChangeVersion(event.target.value)} className="block sm:ml-2 sm:text-sm text-[10px] border px-2 py-1 border-gray-500 rounded bg-white text-black select-none">
                                    ...
                                    <div className="flex mt-2">
                                        <div onClick={() => handleLike(isLike === true ? "unlike" : "like")} className="py-1.5 px-2 round flex items-center cursor-pointer hover:opacity-60 bg-blue-500 text-white 
                                            rounded-lg"><BiLike /><p className="text-[12px] sm:text-[12px] ml-1 select-none">{isLike === false ? "Like" : "Unlike"}</p> 
                                        </div>
                                    </div>
                                    <div className="grow"></div>
                                    <div className="flex mt-2">
                                        <div className="flex items-center mx-1 opacity-70 text-black select-none"><BiLike /><p className="text-[14px] ml-[2px] ">{packageDetail?.likes.length}</p></div>
                                        <div className="flex items-center mx-1 opacity-70 text-black select-none"><BiDownload /><p className="text-[14px] ml-[2px] ">{packageDetail?.downloads}</p></div>
                                    </div>
                                        ...
                                        <div className="w-full sm:block mr-8">
                                            {packageDetail?.version.downloadUrl &&
                                                <button onClick={onDownLoadPackage} className="w-full mt-4 round cursor-pointer hover:opacity-60 bg-emerald-500 text-white 
                                                    ...
                                                    <a className="w-full flex items-center justify-center" href={versionParam === 'latest' ? packageDetail.downloadUrl : currentVersion?.downloadUrl}><p className="text-[14px] sm:text-[14px] lg:text-[16px] mx-2 select-none">Download</p> <BsDownload />
                                                    </a>
                                                </button>
                                            }
                                        </div>
                                        <div className="w-full sm:block mr-8">
                                            {packageDetail?.version.downloadUrl &&
                                                <button className="w-full mt-4 round cursor-pointer hover:opacity-60 text-black select-none-500 border border-black
                                                    px-6 py-2 rounded-lg items-center justify-center">
                                                    <a target="_blank" rel="noopener noreferrer" className="w-full text-black select-none flex items-center justify-center" href={versionParam === 'latest' ? packageDetail.entryUrl : currentVersion?.entryUrl}><p className="text-[14px] sm:text-[14px] lg:text-[16px] mx-2">Preview</p> <CiShare1 />
                                                    </a>
                                                </button>
                                            }
                                        </div>
                            ...
                            <div className="comment-rating my-6 text-black select-none">
                                <div className="flex justify-between">
                                    ...
                                    <button onClick={onClickBtnCommentRating} className="text-sm outline-none hover:opacity-80 flex justify-center w-[30%] px-2 py-2 items-center cursor-pointer rounded border-2 border-black text-black select-none text-[18px]"><div className="text-[18px]"><AiOutlineComment /></div><p className="sm:block sm:ml-2 hidden text-[14px]">Comment & rating</p></button>
                                </div>
